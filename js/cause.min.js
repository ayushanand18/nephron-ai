...
function analyzecause(){
    ...
    var r25=parseFloat($("input[name=q25]:checked").val());
    var r26=parseFloat($("input[name=q26]:checked").val());
    var r27=parseFloat($("input[name=q27]:checked").val());
    var r28=parseFloat($("input[name=q28]:checked").val());
    var r29=parseFloat($("input[name=q29]:checked").val());
    var diab= r1+r2+r3+r4+r5+r6+r7+r8+r9+r10;
    var high= r11+r12+r13+r14+r15+r23+r24;
    var gene= r16;
    var metal= r17+r18+r19+r20+r21+r22;
    var iga= r25+r26+r27+r28+r29;

    var causeset = { "Diabetes": diab, "high blood pressure":high, "Genetics":gene, "Heavy Metal Poisoning":metal, "glomerulonephritis":iga};
    ...
    var evals = max_key;
    if (max>0.1){
        evaluated="Based on the data you provided, we found that the top cause might be <b>"+max_key;
        evs = max_key;
    }else{
        evaluated= "Based on the data you provided, we could not confidently zero down on a particular cause, but we feel it can be <b>"+evals;
    }
    
    // Showing the final evaluated cause
    $(".q29").fadeOut(300,function(){
        $(".result").fadeIn(300);
        $(".result").delay(2000).html("<h4> <b>The Results are out!</b></h4>\
        "+evaluated+"</b>.<br/> <a href='#' class='otherlink'>Although, this can be due to other causes also.</a> <br/>\
        ...
        recomendlink.addEventListener('click',function(){
            window.location.href="laststeps?pass="+evals;
        });
