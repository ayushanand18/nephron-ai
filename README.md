...
----
## Conclusion
How the project fulfills the question?
The project was successful in creating a tri-faced approach to fight chronic kidney disease. Upon large-scale implementation, it will prove to be beneficial because,

+ it is a no-cost, user-friendly solution that can be used by anybody without requiring any installation or setup;
+ if used on a widescale, then 
+ Using even lesser parameters than a physician would require, this solution produced an effective accuracy of 99% in detecting chronic kidney disease over a test data.
+ 	
